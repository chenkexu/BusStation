package hssychargingpole.xpg.com.baidumapdemo.view.signcalendarView;
...
public class CalendarView extends View implements View.OnTouchListener {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (isLastMonth(i)) { //如果是上个月
                // 上个月字体颜色
                color = surface.textOtherColor;
            } else if (isNextMonth(i)) { //如果是下个月
                // 下个月字体颜色
                color = surface.textOtherColor;
            } else if (todayIndex != -1) { //这个月
                // 循环为签到的日期加标记
    ...
    private void drawCellFlag(Canvas canvas, int index, int bgcolor,
                              int textcolor) {
        int x = getXByIndex(index);//第几列（4）
        int y = getYByIndex(index);//第几行（3）
