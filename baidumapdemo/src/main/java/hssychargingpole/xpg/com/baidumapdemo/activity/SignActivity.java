...
public class SignActivity extends AppCompatActivity implements View.OnClickListener, CalendarView.OnItemClickListener {
    ...
    private void initData() {
        my_rmb_data = getSharedPreferences(MY_RMBCost, 0); //得到签到的时间
        /**
         * 时间函数
         */
    ...
    private void initViewOper() {
         ...
         * 判断是否签到
         */
        if (nowTime.equals(thisTime) == true) { //如果已经签到
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.act_qd_bt_qiandao:
                ...
                if (nowTime.equals(thisTime)) { //如果已经签到
                    Toast.makeText(this, "今日已签到！",
                            Toast.LENGTH_SHORT).show();
                } else {
                    String day = thisTime.split("-")[2];
                    my_rmb_data.edit().putString(day, thisTime).commit();//把今天的时间存储到文件
            ...
            case R.id.progressImage:
                this.finish();//退出
                break;
            case R.id.act_qd_iv_left://切换上一月
